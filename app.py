...
@media (max-width: 768px) {
    .fv-mobile-topbar { display: flex !important; }
...
if st.session_state.logged_in:
    ...
    st.markdown(f'''
    <div class="fv-mobile-topbar">
        <div class="fv-mobile-logo">
            <span style="width:7px;height:7px;border-radius:50%;background:#1a56ff;
                display:inline-block;box-shadow:0 0 6px #1a56ff;"></span>
            FinVault
        </div>
        <div class="fv-mobile-page">{st.session_state.current_page.split(" ", 1)[-1] if " " in st.session_state.current_page else st.session_state.current_page}</div>
    </div>
    ''', unsafe_allow_html=True)

    # ── Working mobile bottom nav ──
    bottom_items = [
        ("📊", "Home",   "📊 Dashboard"),
        ("➕", "Add",    "➕ Add Transaction"),
    ...
    b_cols = st.columns(5)
    for i, (icon, label, page) in enumerate(bottom_items):
        with b_cols[i]:
            ...
            if page:
                ...
                # "More" button opens a selectbox overlay
    ...
    # Show full page picker when "More" is tapped
