...
# SUPABASE CONNECTION
# ─────────────────────────────────────────
SUPABASE_URL = "https://lfoheejwfwhlfnhgbryk.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxmb2hlZWp3ZndobGZuaGdicnlrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2MzQ3OTQsImV4cCI6MjA5NjIxMDc5NH0.pJ3tXA0csJmmwVdrU_9MGwBYIXpNvci8w2C43_7IF70"
...
def load_transactions(uid):
    res = db.table("transactions").select("*").eq("user_id", uid).order("date", desc=True).execute()
    rows = res.data
    if rows:
...
def load_budgets(uid, month):
    res = db.table("budgets").select("category,amount").eq("user_id", uid).eq("month", month).execute()
    return {r["category"]: r["amount"] for r in res.data}
        ...
        if not username or not email or not password:
            ...
            try:
                ...
                if existing.data:
                    ...
                    db.table("users").insert({"username": username, "email": email, "password": password}).execute()
...
# ─────────────────────────────────────────
    ...
    if sub:
        res = db.table("users").select("*").eq("email", email).eq("password", password).execute()
...
# ─────────────────────────────────────────
    ...
    # ── Load goals ──
    res_goals = db.table("goals").select("*").eq("user_id", st.session_state.user_id).order("deadline").execute()
    goals = res_goals.data
    ...
    for g in goals:
        ...
        with col_del:
            ...
            if st.button("💾 Save", key=f"gsave_{gid}"):
                db.table("goals").update({"saved": gsaved + add_amt}).eq("id", gid).execute()
                st.success(f"+₹{add_amt:,.0f} added!")
                st.rerun()

        if st.button("🗑️ Delete goal", key=f"gdel_{gid}"):
            db.table("goals").delete().eq("id", gid).execute()
            st.rerun()
...
# ─────────────────────────────────────────
    ...
    res_bills = db.table("bills").select("*").eq("user_id", st.session_state.user_id).order("due_day").execute()
    ...
    def bill_card(b, due_date, days_diff, status):
        ...
        if st.button("Remove", key=f"bdel_{b['id']}]"):
            db.table("bills").delete().eq("id", b["id"]).execute()
            st.rerun()
