...
label, [data-testid="stWidgetLabel"] p { color: #777 !important; font-size: 0.8rem !important; }
...
def init_db():
    with get_conn() as conn:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS transactions (
            ...
            user_id INTEGER, type TEXT NOT NULL, category TEXT NOT NULL,
            amount REAL NOT NULL, date TEXT NOT NULL, description TEXT,
            is_recurring INTEGER DEFAULT 0, recur_freq TEXT DEFAULT '',
            FOREIGN KEY(user_id) REFERENCES users(user_id))""")
        for col, dflt in [
            ("is_recurring", "0"), ("recur_freq", "''"),
            ("currency", "'INR'"), ("amount_inr", "0")
        ]:
            try: conn.execute(f"ALTER TABLE transactions ADD COLUMN {col} {'INTEGER' if col=='is_recurring' else 'TEXT' if col in ('recur_freq','currency') else 'REAL'} DEFAULT {dflt}")
            except: pass
        conn.execute("DROP TABLE IF EXISTS budgets")
        conn.execute("""CREATE TABLE IF NOT EXISTS budgets (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER,
            category TEXT NOT NULL, month TEXT NOT NULL, amount REAL NOT NULL,
            UNIQUE(user_id, category, month),
            FOREIGN KEY(user_id) REFERENCES users(user_id))""")
        conn.execute("""CREATE TABLE IF NOT EXISTS goals (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER,
            name TEXT NOT NULL, icon TEXT DEFAULT '🎯',
            target REAL NOT NULL, saved REAL DEFAULT 0,
            deadline TEXT NOT NULL,
            FOREIGN KEY(user_id) REFERENCES users(user_id))""")
        conn.execute("""CREATE TABLE IF NOT EXISTS bills (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER,
            name TEXT NOT NULL, icon TEXT DEFAULT '💡',
            amount REAL NOT NULL, due_day INTEGER NOT NULL,
    ...
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT id, type, category, amount, date, description,
                   is_recurring, recur_freq,
                   COALESCE(currency,'INR') as currency,
                   COALESCE(amount_inr, amount) as amount_inr
            FROM transactions WHERE user_id=? ORDER BY date DESC""", (uid,)).fetchall()
    if rows:
...
# ─────────────────────────────────────────
    ...
    page_header(f"gm {st.session_state.username} 👋", f"// {datetime.today().strftime('%A, %d %B %Y')}  ·  your financial pulse")
    ...
    c1.markdown(glass_kpi("total income",  f"₹{total_inc:,.0f}", GREEN, "kpi-pill-green", "INCOME"), unsafe_allow_html=True)
    c2.markdown(glass_kpi("total spent",   f"₹{total_exp:,.0f}", RED,   "kpi-pill-red",   "EXPENSE"), unsafe_allow_html=True)
    c3.markdown(glass_kpi("net saved",     f"₹{savings:,.0f}",   GOLD,  "kpi-pill-gold",  "SAVINGS"), unsafe_allow_html=True)
    c4.markdown(glass_kpi("health score",  f"{score}/100",       score_color, "kpi-pill-cyan", "SCORE"), unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if not expense_df.empty:
            ...
            fig_donut.update_layout(
                **plotly_base(320), showlegend=True,
                legend=dict(
        ...
        if not category:
            ...
            with get_conn() as conn:
                conn.execute("""INSERT INTO transactions
                    (user_id,type,category,amount,date,description,is_recurring,recur_freq,currency,amount_inr)
                    VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (st.session_state.user_id, tx_type, category, amount,
                     str(tx_date), description, int(is_rec), rec_freq, cur_code, amount_inr))
                conn.commit()
...
# ─────────────────────────────────────────
    ...
    for _, row in recurring.iterrows():
        ...
        if st.button(f"Log again today", key=f"rl_{row['ID']}"):
            with get_conn() as conn:
                conn.execute("""INSERT INTO transactions
                    (user_id,type,category,amount,date,description,is_recurring,recur_freq)
                    VALUES(?,?,?,?,?,?,?,?)""",
                    (st.session_state.user_id, row["Type"], row["Category"],
                     row["Amount"], str(date.today()), row["Description"], 1, row["Frequency"]))
                conn.commit()
...
# ─────────────────────────────────────────
    ...
    if col3.button("Save", use_container_width=True):
        if not new_cat.strip():
            ...
            with get_conn() as conn:
                conn.execute("""INSERT INTO budgets (user_id,category,month,amount) VALUES(?,?,?,?)
                    ON CONFLICT(user_id,category,month) DO UPDATE SET amount=excluded.amount""",
                    (st.session_state.user_id, new_cat.strip(), month_str, new_amt))
                conn.commit()
...
# ─────────────────────────────────────────
    ...
    if upd:
        with get_conn() as conn:
            conn.execute("UPDATE transactions SET type=?,category=?,amount=?,date=?,description=? WHERE id=?",
                         (new_type, new_cat, new_amt, str(new_date), new_desc, tx_id))
            conn.commit()
        st.success("Updated!")
        st.rerun()
    if dlt:
...
# ─────────────────────────────────────────
    ...
    with st.expander("➕ Add new goal", expanded=False):
        ...
        if g_sub:
            if not g_name:
                ...
                with get_conn() as conn:
                    conn.execute("""INSERT INTO goals (user_id, name, icon, target, saved, deadline)
                        VALUES (?,?,?,?,?,?)""",
                        (st.session_state.user_id, g_name, g_icon, g_target, g_saved, str(g_date)))
                    conn.commit()
    ...
    # ── Load goals ──
    with get_conn() as conn:
        goals = conn.execute("""SELECT * FROM goals WHERE user_id=? ORDER BY deadline""",
    ...
    for g in goals:
        gid, _, gname, gicon, gtarget, gsaved, gdeadline = g["id"], g["user_id"], g["name"], g["icon"], g["target"], g["saved"], g["deadline"]
        ...
        with col_del:
            ...
            if st.button("💾 Save", key=f"gsave_{gid}"):
                with get_conn() as conn:
                    conn.execute("UPDATE goals SET saved=saved+? WHERE id=?", (add_amt, gid))
                    conn.commit()
...
# ─────────────────────────────────────────
    ...
    with st.expander("➕ Add bill reminder", expanded=False):
        ...
        if b_sub:
            if not b_name:
                ...
                with get_conn() as conn:
                    conn.execute("""INSERT INTO bills (user_id, name, icon, amount, due_day, frequency)
                        VALUES (?,?,?,?,?,?)""",
                        (st.session_state.user_id, b_name, b_icon, b_amt, int(b_due), b_freq))
                    conn.commit()
                st.success(f"Bill '{b_name}' reminder saved!")
                st.rerun()

    with get_conn() as conn:
        bills = conn.execute("SELECT * FROM bills WHERE user_id=? ORDER BY due_day",
                             (st.session_state.user_id,)).fetchall()
    ...
    def bill_card(b, due_date, days_diff, status):
        ...
        if st.button("🗑️ Remove", key=f"bdel_{b['id']}"):
            with get_conn() as conn:
                conn.execute("DELETE FROM bills WHERE id=?", (b["id"],))
                conn.commit()
            st.rerun()
        ...
        if amounts:
            ...
            with c2:
                ...
                if r_sub:
                    with get_conn() as conn:
                        conn.execute("""INSERT INTO transactions
                            (user_id,type,category,amount,date,description,currency,amount_inr)
                            VALUES(?,?,?,?,?,?,?,?)""",
                            (st.session_state.user_id, "Expense", r_cat, r_amt,
                             str(r_date), r_desc, "INR", r_amt))
                        conn.commit()
