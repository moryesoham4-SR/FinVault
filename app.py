...
if st.session_state.logged_in:
    ...
    # ── iOS floating nav — pure HTML, uses query param hack for navigation ──
    nav_items = [
        ("📊", "Home",    "📊 Dashboard"),
        ("➕", "Add",     "➕ Add Transaction"),
    ...
    nav_html = '<div class="fv-ios-nav" id="fv-ios-nav">'
    for icon, label, page in nav_items:
        is_active = (page == st.session_state.current_page) if page else False
        active_cls = "active" if is_active else ""
        nav_html += f'''<div class="fv-ios-nav-item {active_cls}" id="nav-{label.lower()}"
            onclick="fvNav('{label}')">
            <div class="fv-ios-nav-icon">{icon}</div>
            <div class="fv-ios-nav-label">{label}</div>
            <div class="fv-ios-nav-dot"></div>
        </div>'''
    nav_html += '</div>'

    nav_html += """
<script>
function fvNav(label) {
    // Find and click the hidden Streamlit button
    var btns = window.parent.document.querySelectorAll('button');
    for (var b of btns) {
        if (b.innerText && b.innerText.trim() === 'NAV_' + label) {
            b.click(); return;
        }
    }
}
    ...
    # Hidden Streamlit buttons that actually do the navigation
    nav_map = {
        "Home":   "📊 Dashboard",
        "Add":    "➕ Add Transaction",
        "Budget": "💰 Budget Planner",
    ...
    hidden_cols = st.columns(len(nav_map) + 1)
    for i, (label, page) in enumerate(nav_map.items()):
        with hidden_cols[i]:
            if st.button(f"NAV_{label}", key=f"nav_hidden_{label}",
                         help=page):
                st.session_state.current_page = page
                st.rerun()

    # "More" button — shows full nav selector
    with hidden_cols[len(nav_map)]:
        if st.button("NAV_More", key="nav_hidden_more"):
    ...
    button[kind="secondary"][data-testid="baseButton-secondary"] {
        /* Only hide our nav buttons — they have NAV_ text */
    }
    [data-testid="column"] button:has-text("NAV_") {
    ...
    if st.session_state.get("show_more_nav", False):
        st.markdown('''<div style="position:fixed;bottom:90px;left:50%;
            transform:translateX(-50%);width:calc(100% - 32px);max-width:380px;
            background:rgba(14,16,28,0.96);backdrop-filter:blur(24px);
            border:1px solid rgba(255,255,255,0.12);border-radius:20px;
            padding:1rem;z-index:99998;
            box-shadow:0 20px 60px rgba(0,0,0,0.7);">''',
            unsafe_allow_html=True)
        more_pick = st.selectbox("Go to page", nav_pages,
                                  index=nav_pages.index(st.session_state.current_page)
                                  if st.session_state.current_page in nav_pages else 0,
                                  label_visibility="collapsed",
                                  key="more_nav_select")
        c1, c2 = st.columns(2)
        if c1.button("Go", use_container_width=True, type="primary"):
            st.session_state.current_page = more_pick
