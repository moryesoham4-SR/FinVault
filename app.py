...
# ─────────────────────────────────────────
    ...
    if submitted:
        if not category:
            ...
            color = "#00c87a" if tx_type == "Income" else "#ff4040"
            icon  = "💰" if tx_type == "Income" else "💸"
            st.markdown(f'''<div class="glass-card" style="border-color:{color}44;">
                <div style="display:flex;align-items:center;gap:14px;">
                    <span style="font-size:2rem;">{icon}</span>
                    <div>
                        <div style="font-weight:700;color:{color};font-size:1rem;">{tx_type} Logged!</div>
                        <div style="font-size:0.78rem;color:#6a7590;font-family:DM Mono,monospace;margin-top:3px;">
                            {amount:,.2f} {cur_code} = ₹{amount_inr:,.2f} &nbsp;·&nbsp;
                            {PM_ICONS.get(payment_mode_clean,"")} {payment_mode_clean} &nbsp;·&nbsp;
                            {category}
                        </div>
                    </div>
                </div>
            ...
            if cur_code != "INR":
                st.caption(f"Rate: 1 {cur_code} = ₹{rate}")
...
# ─────────────────────────────────────────
    ...
    if not fil.empty:
        ...
        st.dataframe(disp[["ID","Type","Category","Amount","Currency","Payment_Mode","Date","Description"]],
