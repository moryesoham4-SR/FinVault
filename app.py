...
/* ══════════════════════════════════════
   SIDEBAR + LAYOUT — ALWAYS HIDDEN
   ══════════════════════════════════════ */

/* Hide sidebar and its toggle on ALL screen sizes */
...
/* Force main content to fill entire screen */
...
/* Desktop: wider padding, hide bottom nav */
...
# SIDEBAR
# ─────────────────────────────────────────
# ── Navigation (no sidebar — top bar only) ──
nav_pages = [
...
if st.session_state.logged_in:
    ...
    col_logo.markdown(
        ...
        'font-size:1.05rem;color:#fff;padding:0.5rem 0;'
        'display:flex;align-items:center;gap:6px;">'
        '<span style="width:8px;height:8px;border-radius:50%;'
        'background:#1a56ff;display:inline-block;box-shadow:0 0 6px #1a56ff;"></span>FinVault</div>',
        unsafe_allow_html=True
    )
    if "current_page" not in st.session_state:
        st.session_state.current_page = "📊 Dashboard"
    menu = col_menu.selectbox("Page", nav_pages,
                               index=nav_pages.index(st.session_state.current_page)
                               if st.session_state.current_page in nav_pages else 0,
                               label_visibility="collapsed")
    st.session_state.current_page = menu
    if col_btn.button("Logout", use_container_width=True):
        st.session_state.logged_in = False
        st.session_state.user_id = None
        st.session_state.username = ""
        st.session_state.current_page = "📊 Dashboard"
        st.rerun()
    st.markdown('<hr style="margin:0.2rem 0 0.8rem;border-color:rgba(255,255,255,0.07);">', unsafe_allow_html=True)

    # ── Working bottom nav (sets session state + reruns) ──
    bottom_nav_items = [
        ("📊", "Dashboard", "📊 Dashboard"),
        ("➕", "Add", "➕ Add Transaction"),
    ...
    st.markdown('<div style="height:70px;"></div>', unsafe_allow_html=True)  # bottom padding
    cols = st.columns(len(bottom_nav_items))
    for i, (icon, label, page) in enumerate(bottom_nav_items):
        is_active = st.session_state.current_page == page
        style = "color:#1a56ff;font-weight:700;" if is_active else "color:#6a7590;"
        with cols[i]:
            st.markdown(f'<div style="text-align:center;font-size:1.2rem;">{icon}</div>', unsafe_allow_html=True)
            if st.button(label, key=f"bnav_{i}", use_container_width=True,
