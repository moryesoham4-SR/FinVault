...
/* ═══════════════════════════════════════════════════
   FINVAULT — BLACK GOLD ELITE THEME
   Luxury · Minimal · Intelligent
   ═══════════════════════════════════════════════════ */
...
[data-testid="block-container"] { padding-top: 0.5rem !important; padding-bottom: 2rem !important; }
[data-testid="stMainBlockContainer"] { max-width: 100% !important; padding: 0 1.5rem !important; }
...
[data-testid="stSidebar"] [role="radiogroup"] label:hover span { color: var(--gold-light) !important; }
...
[data-testid="stProgressBar"] > div {
    background: var(--bg3) !important; border-radius: 999px !important;
}
...
[data-testid="stMainBlockContainer"] {
    animation: fadeUp 0.35s cubic-bezier(0.16,1,0.3,1) !important;
}
@keyframes fadeUp {
...
.fv-ticker-wrap {
    ...
    border-radius: 10px; padding: 7px 0; margin-bottom: 1.2rem;
}
.fv-ticker {
...
.section-tag {
    ...
    font-size: 0.62rem; font-weight: 600; padding: 2px 10px;
    border-radius: 6px; background: var(--gold-dim);
    color: var(--gold); border: 1px solid var(--border);
    letter-spacing: 0.12em; margin-bottom: 3px; text-transform: uppercase;
}
.fv-section-title {
...
/* ── Pills / tags ── */
.kpi-pill { display: inline-block; font-family: 'DM Mono', monospace;
...
.kpi-pill-gold  { background: rgba(212,175,55,0.12); color: var(--gold-light);
    border: 1px solid var(--border); }
.kpi-pill-red   { background: rgba(239,68,68,0.12); color: #EF4444;
    border: 1px solid rgba(239,68,68,0.25); }
.kpi-pill-cyan  { background: rgba(59,130,246,0.12); color: #3B82F6;
    border: 1px solid rgba(59,130,246,0.25); }

/* ── Budget bars ── */
.budget-bar-bg {
    background: var(--bg3); border-radius: 999px;
    height: 6px; margin: 4px 0 12px; overflow: hidden;
}
.budget-bar-fill {
...
.kpi-big {
    ...
    margin: 4px 0 2px; color: var(--gold-light);
}
.kpi-label-sm {
...
.invest-name { font-weight: 600; font-size: 0.9rem; color: var(--text); margin-bottom: 2px; }
.invest-desc { font-size: 0.76rem; color: var(--text-2); }

/* ── Home features ── */
.home-feature {
    background: var(--glass); border: 1px solid var(--border);
    border-radius: 12px; padding: 0.8rem 1rem; margin-bottom: 0.4rem;
    font-size: 0.84rem; color: var(--text); display: flex;
    gap: 10px; align-items: center;
    -webkit-text-fill-color: var(--text);
    transition: border-color 0.2s;
}
.home-feature:hover { border-color: var(--gold); }
.home-feature span { color: var(--gold); }

/* ── Sidebar components ── */
.sidebar-logo {
    font-family: 'Inter', sans-serif; font-size: 1.3rem; font-weight: 800;
    letter-spacing: -0.01em; padding: 0.5rem 0 0.1rem;
    background: linear-gradient(90deg, var(--gold), var(--gold-light));
...
.ai-insight-icon { font-size: 1.4rem; }
.ai-insight-title { font-weight: 600; font-size: 0.88rem; color: var(--text); }
.ai-insight-val { font-family: 'DM Mono', monospace; font-size: 1rem;
    font-weight: 600; color: var(--gold-light); }
.ai-insight-desc { font-size: 0.74rem; color: var(--text-2); margin-top: 2px; }

/* ═══════════════════════════════════════════
   RESPONSIVE
   ═══════════════════════════════════════════ */
@media (max-width: 768px) {
    /* Keep sidebar accessible via hamburger on mobile */
    [data-testid="stSidebar"] { display: flex !important; }
    [data-testid="collapsedControl"] { display: flex !important; }
    /* Style the hamburger button to be visible */
    ...
    [data-testid="stMainBlockContainer"] { padding: 0 0.6rem !important; padding-bottom: 1rem !important; }
    .fv-hero-stats { grid-template-columns: repeat(2, 1fr) !important; }
    .fv-hero { padding: 1.1rem 1.2rem !important; border-radius: 18px !important; }
    .fv-page-title { font-size: 1.3rem !important; }
...
CYAN     = "#00dcff"
PURPLE   = "#E3C15B"
PALETTE  = ["#D4AF37","#10B981","#E3C15B","#EF4444","#3B82F6","#F59E0B","#B8962E","#B5B5BD"]
...
if st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        st.markdown('<div class="sidebar-tagline">PREMIUM PERSONAL FINANCE</div>', unsafe_allow_html=True)
...
# ─────────────────────────────────────────
    ...
    page_header("Export Reports", "// your financial story")
    df = load_transactions(st.session_state.user_id)
    if df.empty:
    ...
    months = sorted(disp["Month"].unique().tolist(), reverse=True)

    col_sel, col_all = st.columns([2,1])
    sel = col_sel.selectbox("Select Month", months,
                             format_func=lambda m: datetime.strptime(m, "%Y-%m").strftime("%B %Y"))

    mdf = disp[disp["Month"]==sel].copy()
    mdf["Date"] = mdf["Date"].dt.strftime("%d %b %Y")
    inc  = mdf[mdf["Type"]=="Income"]["Amount_INR"].sum()
    exp  = mdf[mdf["Type"]=="Expense"]["Amount_INR"].sum()
    sav  = inc - exp
    sav_pct = (sav/inc*100) if inc > 0 else 0
    sel_label = datetime.strptime(sel, "%Y-%m").strftime("%B %Y")

    # ── Summary cards ──
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        f'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:1.2rem;">'
        f'<div class="glass-card" style="border-top:2px solid #10B981;padding:1rem;">'
        f'<div style="font-size:0.62rem;color:#10B981;font-family:DM Mono,monospace;letter-spacing:0.1em;margin-bottom:4px;">INCOME</div>'
        f'<div style="font-size:1.5rem;font-family:DM Mono,monospace;font-weight:600;color:#10B981;">₹{inc:,.0f}</div>'
        f'<div style="font-size:0.68rem;color:#6B6B75;">{sel_label}</div></div>'
        f'<div class="glass-card" style="border-top:2px solid #EF4444;padding:1rem;">'
        f'<div style="font-size:0.62rem;color:#EF4444;font-family:DM Mono,monospace;letter-spacing:0.1em;margin-bottom:4px;">EXPENSE</div>'
        f'<div style="font-size:1.5rem;font-family:DM Mono,monospace;font-weight:600;color:#EF4444;">₹{exp:,.0f}</div>'
        f'<div style="font-size:0.68rem;color:#6B6B75;">{sel_label}</div></div>'
        f'<div class="glass-card" style="border-top:2px solid #D4AF37;padding:1rem;">'
        f'<div style="font-size:0.62rem;color:#D4AF37;font-family:DM Mono,monospace;letter-spacing:0.1em;margin-bottom:4px;">SAVINGS</div>'
        f'<div style="font-size:1.5rem;font-family:DM Mono,monospace;font-weight:600;color:#D4AF37;">₹{sav:,.0f}</div>'
        f'<div style="font-size:0.68rem;color:#6B6B75;">{sav_pct:.1f}% rate</div></div>'
        f'</div>',
        unsafe_allow_html=True
    )

    # ── Transaction table ──
    st.markdown('<div class="section-tag">TRANSACTIONS</div><div class="fv-section-title">Transaction Log</div>', unsafe_allow_html=True)
    show_cols = [c for c in ["ID","Type","Category","Amount_INR","Payment_Mode","Date","Description"] if c in mdf.columns]
    st.dataframe(mdf[show_cols].rename(columns={"Amount_INR":"Amount (₹)"}),
                 use_container_width=True, hide_index=True)

    # ── Export buttons ──
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-tag">EXPORT</div><div class="fv-section-title">Download Reports</div>', unsafe_allow_html=True)
    ec1, ec2, ec3 = st.columns(3)

    # CSV - monthly
    buf_m = io.StringIO(); mdf.to_csv(buf_m, index=False)
    ec1.download_button(f"CSV — {sel_label}", buf_m.getvalue(),
                        file_name=f"finvault_{st.session_state.username}_{sel}.csv",
                        mime="text/csv", use_container_width=True)

    # CSV - all time
    all_df = df.copy(); all_df["Date"] = all_df["Date"].dt.strftime("%d %b %Y")
    buf_a = io.StringIO(); all_df.to_csv(buf_a, index=False)
    ec2.download_button("CSV — All Time", buf_a.getvalue(),
                        file_name=f"finvault_{st.session_state.username}_all.csv",
                        mime="text/csv", use_container_width=True)
    ...
    try:
        ...
        def make_pdf():
            ...
            C_GOLD2 = rl_colors.HexColor("#E3C15B")
            C_GREEN = rl_colors.HexColor("#10B981")
            C_RED   = rl_colors.HexColor("#EF4444")
            C_TEXT  = rl_colors.HexColor("#F8FAFC")
            C_MUTED = rl_colors.HexColor("#B5B5BD")
            C_DARK  = rl_colors.HexColor("#151518")
            ...
            story.append(Paragraph("🏦 FinVault", title_style))
            story.append(Paragraph("Premium Personal Finance Report", sub_style))
            story.append(Paragraph(f"{sel_label}  ·  {st.session_state.username}", sub_style))
            story.append(HRFlowable(width="100%", thickness=1, color=C_GOLD, spaceAfter=16))

            # Summary table
            summary_data = [
                ["METRIC", "VALUE"],
                ["Total Income", f"Rs {inc:,.2f}"],
                ["Total Expense", f"Rs {exp:,.2f}"],
                ["Net Savings", f"Rs {sav:,.2f}"],
                ["Savings Rate", f"{sav_pct:.1f}%"],
                ["Transactions", str(len(mdf))],
            ]
            summary_style = TableStyle([
                ...
                ("FONTSIZE",   (0,1), (-1,-1), 10),
                ("ROWBACKGROUNDS", (0,1), (-1,-1), [C_DARK, rl_colors.HexColor("#1C1C20")]),
                ("GRID", (0,0), (-1,-1), 0.5, rl_colors.HexColor("#2a2a2a")),
            ...
            # Top categories
            if not mdf[mdf["Type"]=="Expense"].empty:
                cats = mdf[mdf["Type"]=="Expense"].groupby("Category")["Amount_INR"].sum().sort_values(ascending=False).head(5)
                story.append(HRFlowable(width="100%", thickness=0.5, color=C_GOLD, spaceAfter=8))
                story.append(Paragraph("Top Spending Categories", ParagraphStyle("h2",
                    parent=styles["Normal"], fontSize=13, fontName="Helvetica-Bold",
                    textColor=C_GOLD, spaceAfter=8)))
                cat_data = [["CATEGORY", "AMOUNT", "% OF EXPENSE"]]
                for cat, amt in cats.items():
                    pct = amt/exp*100 if exp > 0 else 0
                    cat_data.append([cat, f"Rs {amt:,.2f}", f"{pct:.1f}%"])
                ct = Table(cat_data, colWidths=[8*cm, 5*cm, 4*cm])
                ct.setStyle(summary_style)
                story.append(ct)
                story.append(Spacer(1, 0.6*cm))

            # Transaction list
            story.append(HRFlowable(width="100%", thickness=0.5, color=C_GOLD, spaceAfter=8))
            story.append(Paragraph("Transaction Details", ParagraphStyle("h2",
                parent=styles["Normal"], fontSize=13, fontName="Helvetica-Bold",
                textColor=C_GOLD, spaceAfter=8)))

            tx_data = [["DATE", "TYPE", "CATEGORY", "AMOUNT", "MODE"]]
            for _, row in mdf.head(50).iterrows():
                pm = row.get("Payment_Mode", "Cash") if "Payment_Mode" in row else "Cash"
                tx_data.append([
                    str(row["Date"]),
                    str(row["Type"]),
                    str(row["Category"])[:18],
                    f"Rs {row['Amount_INR']:,.2f}",
                    str(pm)[:12],
                ])
            ...
            tt = Table(tx_data, colWidths=[3*cm, 2.5*cm, 5*cm, 4*cm, 3*cm])
            tt.setStyle(tx_style)
            story.append(tt)
            ...
            story.append(Paragraph(
                f"Generated by FinVault  ·  {datetime.today().strftime('%d %B %Y')}  ·  Black Gold Elite",
        ...
        pdf_bytes = make_pdf()
        ec3.download_button(
            f"PDF Report — {sel_label}",
            pdf_bytes,
            file_name=f"finvault_{st.session_state.username}_{sel}.pdf",
            mime="application/pdf",
            use_container_width=True
        )
    except ImportError:
        ec3.warning("Install `reportlab` for PDF export")
    except Exception as ex:
        ec3.error(f"PDF error: {ex}")
